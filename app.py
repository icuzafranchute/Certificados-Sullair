...
def clasificar(un, art, interno):
    """
    ERROR 1 FIX: Clasificar por U.N. primero (columna B del Excel).
    Fallback por prefijo de interno, luego por artículo.
    """
    # 1. Por Unidad de Negocio (más confiable)
    un_str = str(un).strip().split()[0] if pd.notna(un) and str(un).strip() else ''
    if un_str in UN_MAP:
        return UN_MAP[un_str]

    # 2. Por prefijo de interno (internos estándar E0x/A0x)
    ...
    # 3. Internos viejos: 3xxx = Plataforma, 4xxx = Manipulador (4 dígitos, solo números)
    if re.match(r'^3\d{3}$', i): return 'Plataforma'
    if re.match(r'^4\d{3}$', i): return 'Manipulador'

    # 4. Por artículo (FAMILIA_MAP)
    FAMILIA_MAP = {
        '70R':'Generador','120R':'Generador','45KVA':'Generador','A120R':'Generador',
...
def parsear_periodo(periodo_raw):
    """
    ERROR 4 FIX: Devuelve 'Julio 2025' a partir de 'Julio', 'Julio 2025', etc.
    Si no tiene año, agrega el año actual.
    """
    raw = str(periodo_raw).strip()
    # Ya tiene año (4 dígitos)
    if re.search(r'\d{4}', raw):
        return raw
    # Solo tiene mes — agregar año actual
    return f"{raw} {datetime.today().year}"
...
# ─────────────────────────────────────────────────────────────────────────────
# GENERAR EXCEL — UNA HOJA POR CERTIFICADO
# ─────────────────────────────────────────────────────────────────────────────
def crear_hoja_xlsx(ws, cert_num, sucursal, direccion, periodo,
    ...
    # ERROR 3 FIX: columnas según mostrar_precios
    if mostrar_precios:
        hdrs=[(3,'Equipo',Border(top=med(),bottom=med(),left=med(),right=thin())),
        ...
        hdrs=[(3,'Equipo',Border(top=med(),bottom=med(),left=med(),right=thin())),
              ...
              (6,'Inicio Periodo',Border(top=med(),bottom=med(),left=thin(),right=thin())),
              (7,'Fin Periodo',Border(top=med(),bottom=med(),left=thin(),right=thin())),
              (8,'Dias',Border(top=med(),bottom=med(),left=thin(),right=med()))]
        # Merge I-J en header
        ws.merge_cells('I12:J12'); ws['I12'].border=Border(top=med(),bottom=med(),right=med())

    for col_n,txt,brd in hdrs:
    ...
    for col_n,v,brd in zip(range(3,3+len(dat_vals)),dat_vals,dat_borders):
        c=ws.cell(13,col_n,v); c.font=fd; c.alignment=ac; c.border=brd
        if mostrar_precios and col_n in(9,10): c.number_format=num_fmt

    # Fila 14 subtotal
    last_col = 10 if mostrar_precios else 8
    for col_n in range(3, last_col-1):
        c=ws.cell(14,col_n); c.font=Font(name='Calibri',size=11)
...
# ─────────────────────────────────────────────────────────────────────────────
# GENERAR PDF — UN ARCHIVO POR CERTIFICADO
# ─────────────────────────────────────────────────────────────────────────────
def crear_pdf(cert_num, sucursal, direccion, periodo, cliente,
    ...
    BOX_T=PAGE_H-2.0*cm; BOX_B=PAGE_H-15.8*cm
    BOX_W=BOX_R-BOX_L
    ...
    # Logo
    if os.path.exists(LOGO_PATH):
        c.drawImage(LOGO_PATH, BOX_R-4.6*cm-0.35*cm, BOX_T-1.35*cm-0.55*cm,
                    width=4.6*cm, height=1.35*cm, preserveAspectRatio=True, mask='auto')

    # Título
    TX=BOX_L+0.65*cm
    c.setFont('Helvetica-Bold',11.5); c.setFillColor(colors.black)
    c.drawString(TX,BOX_T-1.15*cm,'CERTIFICACIÓN DE SERVICIOS')
    c.setFont('Helvetica',9.5); c.drawString(TX,BOX_T-1.75*cm,'Alquiler de equipos')

    SEP_Y=BOX_T-2.2*cm; c.setLineWidth(0.5); c.line(BOX_L,SEP_Y,BOX_R,SEP_Y)

    LBL_X=TX; VAL_X=TX+3.2*cm; MID_X=BOX_L+BOX_W*0.50; RH=0.58*cm
    Y0=SEP_Y-0.68*cm

    def lbl(y,txt): c.setFont('Helvetica-Bold',9.5); c.setFillColor(colors.black); c.drawString(LBL_X,y,txt)
    def val(y,txt,x=None): c.setFont('Helvetica',9.5); c.setFillColor(colors.black); c.drawString(x or VAL_X,y,str(txt))

    lbl(Y0,'Certificado N°:');        val(Y0,str(cert_num));           val(Y0,f'Sucursal {sucursal}',x=MID_X)
    lbl(Y0-RH,'Cliente:');            val(Y0-RH,cliente);              val(Y0-RH,direccion,x=MID_X)
    lbl(Y0-2*RH,'Periodo:');          val(Y0-2*RH,periodo)
    lbl(Y0-3*RH,'CVU:')
    c.setFont('Helvetica-Bold',9.5);  c.drawString(MID_X,Y0-3*RH,'Fecha Certif.:')
    c.setFont('Helvetica',9.5);       c.drawString(MID_X+3.05*cm,Y0-3*RH,datetime.today().strftime('%d/%m/%Y'))

    # Tabla
    TAB_TOP=SEP_Y-3.45*cm; TAB_L=BOX_L+0.35*cm; TAB_W=BOX_W-0.7*cm

    if mostrar_precios:
        CW=[TAB_W*0.23,TAB_W*0.10,TAB_W*0.13,TAB_W*0.13,TAB_W*0.13,TAB_W*0.07,TAB_W*0.13,TAB_W*0.08]
        hdrs=['Equipo','Modelo','Interno','Desde','Hasta','Dias','Valor diario','Total']
        dat=[equipo,articulo,interno,fmt_fecha(inicio),fmt_fecha(fin),
             cnt_dias,f'{moneda} {precio:,.2f}',f'{moneda} {total:,.2f}']
    else:
        CW=[TAB_W*0.28,TAB_W*0.14,TAB_W*0.18,TAB_W*0.15,TAB_W*0.15,TAB_W*0.10]
        hdrs=['Equipo','Modelo','Interno','Desde','Hasta','Dias']
        dat=[equipo,articulo,interno,fmt_fecha(inicio),fmt_fecha(fin),cnt_dias]

    data=[hdrs,dat,['']*(len(hdrs))]
    t=Table(data,colWidths=CW,rowHeights=[16,15,9])
    t.setStyle(TableStyle([
        ('BACKGROUND',(0,0),(-1,0),VERDE_RL),('TEXTCOLOR',(0,0),(-1,0),colors.white),
        ('FONTNAME',(0,0),(-1,0),'Helvetica-Bold'),('FONTSIZE',(0,0),(-1,0),8),
        ('ALIGN',(0,0),(-1,0),'CENTER'),('VALIGN',(0,0),(-1,0),'MIDDLE'),
        ('FONTNAME',(0,1),(-1,1),'Helvetica'),('FONTSIZE',(0,1),(-1,1),8),
        ('ALIGN',(1,1),(-1,1),'CENTER'),('ALIGN',(0,1),(0,1),'LEFT'),
        ('LEFTPADDING',(0,1),(0,1),4),('VALIGN',(0,1),(-1,1),'MIDDLE'),
        ('BOX',(0,0),(-1,-1),0.8,colors.black),('INNERGRID',(0,0),(-1,-1),0.5,colors.black),
    ]))
    t.wrapOn(c,TAB_W,200); t.drawOn(c,TAB_L,TAB_TOP-t._height)

    # Firma y texto exterior
...
def procesar(archivo_bytes, cert_inicio, periodo_manual, sucursal, direccion,
    ...
    if 'Nombre' in datos.columns:
        ...
        if len(clientes): cliente_nombre = limpiar(str(clientes[0]).strip())
    ...
    for idx,(_, row) in enumerate(datos.iterrows()):
        ...
        # ERROR 2 FIX: nombre "Interno-Empresa-Periodo"
        nombre_base = limpiar(f"{int_.strip()}-{cliente_nombre}-{periodo}")
        ...
        log_lines.append(f"✅ Cert {cert_num:04d} | {eq:<20} | {int_:<12} | {art}")
        cert_num+=1; generados+=1
        prog_bar.progress(int((idx+1)/total_f*100))
        status_txt.text(f"Generando {idx+1} de {total_f}...")

    # Nombre del archivo de descarga: Certificación - CLIENTE - Período
...
c3,c4=st.columns(2)
with c3:
    # ERROR 3 FIX: toggle de precios
    mostrar_precios=st.toggle("Incluir precios en el certificado",value=True,
        help="Desactivar si el cliente no debe ver los valores")
with c4:
    # ERROR 2 FIX: formato de salida
...
if archivo:
    ...
    with st.expander("👁️ Vista previa — primeros 5 registros"):
        cols_ok=[c for c in ['U.N.','Artículo','Interno','Cnt Días','Inicio Período','Fin Período','Pcio Unitario','Total'] if c in datos_prev.columns]
...
if "resultado" in st.session_state:
    ...
    icon="📥 Descargar Excel" if r["ext"]=="xlsx" else "📦 Descargar ZIP (PDFs)"
    st.download_button(label=icon,data=r["bytes"],file_name=r["nombre_descarga"],mime=mime)
    st.markdown('</div>',unsafe_allow_html=True)

st.markdown("---")
st.markdown('<p style="text-align:center;color:#aaa;font-size:.72rem">Sullair Argentina S.A. · Generador Universal de Certificados · v5.0</p>',unsafe_allow_html=True)
